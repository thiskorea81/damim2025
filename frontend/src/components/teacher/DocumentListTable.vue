<script setup>
  import { FileText, Trash2, CheckCircle, XCircle, Clock, AlertCircle, Plane } from 'lucide-vue-next';
      ...
      <table class="doc-table">
        ...
        <tbody>
          <tr v-for="doc in documents" :key="doc.id" @click="$emit('select', doc)" class="cursor-pointer group hover:bg-blue-50">
            ...
            <td class="text-center">
              <div v-if="doc.type === '체험학습'" class="inline-flex items-center gap-1 text-green-700 font-bold text-sm bg-green-50 px-2 py-0.5 rounded border border-green-100">
                <Plane class="w-3 h-3" /> 체험학습
              </div>
              <div v-else>
            ...
            <td class="text-left">
              <div v-if="doc.type === '체험학습'">
                <div class="truncate max-w-[200px] text-gray-800 font-medium group-hover:text-blue-600">
                  {{ doc.plan?.location || doc.reason }}
                </div>
                <div class="text-xs text-gray-400 mt-0.5">
                  {{ doc.period?.start }} ~ {{ doc.period?.end }} ({{ doc.period?.totalDays }}일)
                </div>
              </div>
              <div v-else>
