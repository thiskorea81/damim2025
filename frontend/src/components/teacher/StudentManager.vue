...
const parseInputText = (text) => {
  ...
  lines.forEach(line => {
    ...
    // 첫 번째 컬럼(학번 or 번호) 분석
    const col1 = cols[0]; 
    let grade, classNum, numberStr, studentId;

    // [수정] 5자리 학번인 경우 (예: 10501 -> 1학년 05반 01번)
    if (col1.length === 5 && !isNaN(col1)) {
      grade = col1.substring(0, 1);
      classNum = col1.substring(1, 3); // '05'
      numberStr = col1.substring(3, 5); // '01'
      studentId = col1;
    } 
    // 기존 방식 (번호만 입력된 경우 -> 담임 학급 정보 사용)
    else {
      grade = props.teacherData.assignedGrade;
      classNum = props.teacherData.assignedClass;
      numberStr = col1.padStart(2, '0');
      studentId = `${grade}${classNum}${numberStr}`;
    }
