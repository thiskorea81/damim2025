<template>
  <nav class="flex gap-3 mb-6 overflow-x-auto pb-2 [&::-webkit-scrollbar]:hidden select-none">
    ...
    <template v-if="userRole !== 'teacher'">
      <button v-if="settings?.showAppStatus" @click="$emit('update:activeTab', 'history')" :class="[baseTabClass, activeTab === 'history' ? activeStudentClass : inactiveClass]">
      ...
      <button v-if="settings?.showAbsence" @click="$emit('update:activeTab', 'absence')" :class="[baseTabClass, activeTab === 'absence' ? activeStudentClass : inactiveClass]">
        <FileText class="w-4 h-4 mr-2" /> 결석신고서
      </button>
      <button v-if="settings?.showFieldTrip" @click="$emit('update:activeTab', 'trip_app')" :class="[baseTabClass, activeTab === 'trip_app' ? activeStudentClass : inactiveClass]">
    ...
    <template v-if="userRole === 'teacher'">
      ...
      <button @click="$emit('update:activeTab', 'settings')" :class="[baseTabClass, activeTab === 'settings' ? activeTeacherClass : inactiveClass]">
...
<script setup>
import { FileText, Send, ClipboardList, BookOpen, CheckCircle, List, Users, Settings, Trophy } from 'lucide-vue-next';

defineProps({
  activeTab: String,
  userRole: String,
  settings: Object // [추가] 설정값 받기
});
defineEmits(['update:activeTab']);

const baseTabClass = 'flex items-center px-5 py-3 rounded-xl text-sm font-bold whitespace-nowrap shadow-sm transition-all duration-200 outline-none';
